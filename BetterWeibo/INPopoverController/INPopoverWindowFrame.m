...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	if (self.topHighlightColor) {
        ...
        NSBezierPath *innerPath = [self _popoverHighlightBezierPathWithRect:bounds];
...
- (NSBezierPath *)_popoverHighlightBezierPathWithRect:(NSRect)aRect
{
	const CGFloat radius = self.cornerRadius;
	const CGFloat arrowWidth = self.arrowSize.width-2;
	//const CGFloat arrowHeight = (self.arrowDirection == INPopoverArrowDirectionLeft || self.arrowDirection == INPopoverArrowDirectionRight)?self.arrowSize.height-2:self.arrowSize.height;
    const CGFloat arrowHeight = self.arrowSize.height;
	const CGFloat inset = radius + arrowHeight;
	const NSRect drawingRect = NSInsetRect(aRect, inset+1, inset+1);
	const CGFloat minX = NSMinX(drawingRect);
	const CGFloat maxX = NSMaxX(drawingRect);
	const CGFloat minY = NSMinY(drawingRect);
	const CGFloat maxY = NSMaxY(drawingRect);
